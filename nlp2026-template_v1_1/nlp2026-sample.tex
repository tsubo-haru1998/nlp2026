...
\item 余白の大きさ
\item 行間\footnote{複数行にわたる見出しについては付録\ref{sec:long_headings}を参照．}，行数，文字数（特に\code{baselinestretch}を変更しないこと） $\rightarrow$ 1ページの行数は45行, 各行の文字数は全角23文字である．
\end{itemize}
...
\begin{quote}
\verb|\documentclass[|\\
\verb|  platex, dvipdfmx, english]{nlp2026}|
\end{quote}
...
\newpage
\section*{謝辞}
上述の通り，謝辞は参考文献と同様に，本文の4ページの中に含まなくても構わない（5ページ目に書くことができる）．
